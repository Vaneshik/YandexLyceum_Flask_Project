...
all_categories_href = [elem.findChildren("a")[0].get_attribute_list("href")[0].replace(main_url, "") for elem in
                       all_categories]
filtered_categories_href = [elem for elem in all_categories_href if
                            len(list(
                                filter(lambda x: x, [href.startswith(elem) for href in all_categories_href]))) == 1]
...
for url in filtered_categories_href:
    try:
        ...
        pages_amount = min(int(
            re.findall(r"всего ([0-9]+) страниц", cur_parser.find_all("div", {"class": "results"})[0].text)[0]), 5)
        ...
        for page_number in range(1, pages_amount + 1):
            try:
                ...
                aTagObjects = [elem.findChildren("a")[0] for elem in parser2.find_all("h6", {"class": "card-title"})]
                for aTagObject in aTagObjects:
                    try:
                        ...
                        ITEM_PRICE = int(parser3.find_all("meta", itemprop="price")[0].get_attribute_list("content")[0])
                        ITEM_AMOUNT = random.randint(1, 100)
                        ITEM_PIC_URL = parser3.find_all("meta", property="og:image")[0].get_attribute_list("content")[0]
                        ITEM_CATEGORY = \
                            parser3.find_all("meta", property="product:category")[0].get_attribute_list("content")[0]
